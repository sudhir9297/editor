...
function buildSkylightGeometry(
  ...
  const hasCurb = node.curb ?? true
  const curbH = hasCurb ? Math.max(0, node.curbHeight ?? 0.1) : 0

  // Outer frame dimensions (frame surrounds the glass opening)
  const outerW = w + 2 * ft
  const outerH = h + 2 * ft

  // Frame: outer box minus inner box (the glass opening)
  const outerBox = new THREE.BoxGeometry(outerW, fd + curbH, outerH)
  const innerBox = new THREE.BoxGeometry(w, fd + curbH + 0.02, h)
  ...
  // Offset frame vertically: curb rises above surface, frame depth goes below
  frameGeo.translate(0, curbH / 2, 0)

  // Now trim frame against the roof segment
