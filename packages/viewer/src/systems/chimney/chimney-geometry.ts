...
import { csgEvaluator, csgGeometry, prepareBrushForCSG } from '../../lib/csg-utils'
import { getRoofSegmentBrushes } from '../roof/roof-system'
...
export function buildCapGeometry(node: ChimneyNode, segment: RoofSegmentNode): THREE.BufferGeometry {
  const peakY = segment.wallHeight + (segment.roofType === 'flat' ? 0 : segment.roofHeight)
  const baseY = peakY + node.heightAboveRidge + 0.05
  ...
  if (useFlueHoles) {
    const xs = flueXPositions(
      flueCountForCap, node.width, flueDiameterForCap, node.flueSpacing ?? 1,
    )
    const flueShape: 'round' | 'square' = node.flueShape ?? 'round'
    for (const x of xs) {
      const cg: THREE.BufferGeometry =
        flueShape === 'round'
          ? new THREE.CylinderGeometry(flueInnerForCap / 2, flueInnerForCap / 2, t + 0.04, 24, 1, false)
...
export function buildFluesGeometry(
  node: ChimneyNode,
...
function buildChimneyBodyGeometry(
  ...
  if (style === 'tapered') {
    if (isRound) {
      ...
      pushSlabFaces(positions, uvs, baseY, baseY + sh + EPS, w / 2 + ext, d / 2 + ext, w / 2, d / 2)
...
function buildChimneyBandsGeometry(
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (isRound) {
      ...
      pushSlabFaces(positions, uvs, bandBot, bandTop, w / 2 + bandExt, d / 2 + bandExt, w / 2 + bandExt, d / 2 + bandExt)
    }
  }
